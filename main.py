...
while True:
    try:
        ...
        if ret:
            ...
            seg_map_3c = np.zeros(
                (np.array(frame).shape[0], np.array(frame).shape[1], 3), dtype=np.int8
            )
            ...
            if enable_blur:  # blurs the background, does not add background image
                ...
                mask_image = np.where(
                    seg_map_3c == np.array([15, 15, 15]), frame, blurred_image
                )
            else:  # adds background image, does not blur the background
                mask_image = np.where(
                    seg_map_3c == np.array([15, 15, 15]), frame, background
                )
            ...
            print(
                "Processing frame: {0} at{1:3d} fps. Press ctrl+c to stop".format(
                    count, int(1 / (time.time() - start_time))
                )
            )
